  ...
  const fetchSuggestionsResults = useCallback(async (query: string) => {
    ...
    try {
      ...
      if (Array.isArray(data)) {
        ...
        const transformedLaptops: Laptop[] = data.map((item: SuggestionItem) => ({
          _id: item.id,
          name: item.title,
          laptop_id: item.id,
          img_link: item.image,
          ...
          rating: 0, // suggestions API doesn't provide rating
          laptop_brand: item.brand || "",
          os: "", // suggestions API doesn't provide OS
        }));

        setLaptops(transformedLaptops);
        // Set pagination for suggestions (no real pagination from API)
        setPagination({
          total: transformedLaptops.length,
          page: 1,
          limit: transformedLaptops.length,
          totalPages: 1,
          hasNext: false,
          hasPrev: false,
        });
  ...
  const fetchAdvancedSearchResults = useCallback(
    async (query: string, currentFilters: Filters, page = 1) => {
      ...
      try {
        ...
        // Format query with && separators for multiple terms
        if (query) {
                    ...
                    {laptops.map((laptop, index) => (
                      ...
                      >
                        <div className="relative overflow-hidden">
                          <img
                            src={laptop.img_link || "/placeholder-laptop.jpg"}
                            alt={laptop.name}
                        ...
                        <div className="p-6">
                          ...
                          <div className="grid grid-cols-2 gap-3 text-sm text-white/80 mb-6">
                            ...
                            <div className="flex items-center space-x-2">
                              ...
                              <span className="truncate">{laptop.os}</span>
                            </div>
                          </div>
